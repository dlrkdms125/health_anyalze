...
<br>
ECG 신호를 입력으로 받아 최대 신호값을 측정하고, 설정한 임계값보다 높으면 R파가 감지된 것으로 감지하여 해당 값을 시리얼 모니터에 출력하는 아두이노 코드
<br>
아날로그 핀(A0)에서 읽은 값이 임계값(150)을 초과할 때마다 펄스 간격을 계산하여 시리얼로 출력하는 코드.

### 호흡 데이터
호흡주기 - 임피던스 센서를 사용하여 들이마시는 호흡과 내뱉는 호흡을 감지하고, 각각의 지속 시간을 측정하여 출력하는 코드
호흡 속도 측정 - 임피덴스 센서를 사용하여 호흡윽 감지하고, 매 초마다 1분당 호흡 속도를 계산하여 출력하는 코드
호흡량 측정 - 1초마다 들이마신 호흡량과 내뱉는 호흡량의 평균을 계산하여 출력하는 코드
